...
async def driver_arrived_to_factory(update: Update, context: CustomContext):
    ...
    *args, task_id = data.split('-')
    ...
    # check task car is not milk tanker
    if await task.is_task_car_milktanker:
        ...
        await bot_send_message(
            query,
            context,
            await drivers_next_address_string(update.effective_chat.id, address),
            reply_markup=markup
        )
        return
...
async def driver_arrived_to_depot(update: Update, context: CustomContext):
    ...
    *args, taskevent_id = data.split('-')
    ...
    if await task.is_task_car_milktanker:
        ...
        text = await car_in_farm_string_for_driver(update.effective_user.id)
        i_get = InlineKeyboardButton(
            text=await get_word_driver('i received it', query),
