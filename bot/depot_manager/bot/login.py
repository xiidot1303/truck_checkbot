...
from bot.services.depot_manager_service import create_depot_manager
...
async def get_lang(update: Update, context: CustomContext):
    ...
    # create driver
    await create_depot_manager(
        query.message.chat.id, depot, lang, query.message.chat.first_name
    )
    await query.answer()
